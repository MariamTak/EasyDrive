...
const SignUp = ({ navigation }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch('http://10.0.2.2:8055/api/auth/signup', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        navigation.navigate('Container', { user_id: data.user_id,fullName: fullName, email: email, phoneNumber: phoneNumber, password: password});
      } else {
